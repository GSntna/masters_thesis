...
Finally the data is added to the sqlite database
...
# Get metadata to stick to db schema
metadata = sqlalchemy.MetaData()
metadata.reflect(bind=engine)
properties_table = metadata.tables['properties_raw']
...
if __name__ == '__main__':
    ...
    df.to_sql(
        name='properties_raw',
        con=engine,
        if_exists='replace',  # or 'append' to keep previous data
        index=False,
        dtype={col.name: col.type for col in properties_table.columns}
    )
